...
import { FundingCauseClosedHandler } from './cause-closed.handler';
...
describe('FundingCauseClosedHandler', () => {
  let handler: FundingCauseClosedHandler;
  let repo: jest.Mocked<
    Pick<FundingActionRepository, 'findAllByCauseId' | 'save'>
  >;
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: FundingActionRepository, useValue: repo },
      ],
    }).compile();

    handler = module.get(FundingCauseClosedHandler);
  });
